    ...
    * Async download of prices for a range of days
    ...
    def process_state_and_attributes(self, utc_now: datetime) -> bool:
        ...
        def _isFestive():
            ...
            formatDate = "%d-%m"
            formatFullDate = "%d-%m-%Y"

            if actual_time.strftime(formatDate) in festives:
                return True
            elif actual_time.strftime(formatFullDate) in ss:
                return True
            else:
                return False

        dayOfWeek = actual_time.weekday()
        period = "error"
        if actual_time.hour < 8:
            period = "valley"
        elif dayOfWeek == 6 or dayOfWeek == 0 or _isFestive():
            period = "valley"
